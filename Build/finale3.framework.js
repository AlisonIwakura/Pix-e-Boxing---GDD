...
// end include: preamble.js
  ...
  function _SocketEntrarLobby(jsonPtr) {
      var socket = window.socket;
      if (!socket) return;
  
      var json = UTF8ToString(jsonPtr);
      try {
        socket.emit("entrar_lobby", JSON.parse(json));
      } catch (e) {
        console.error("SocketEntrarLobby JSON invalido:", json, e);
      }
    }

  function _SocketInit() {
      if (typeof io === "undefined") {
        console.error("❌ socket.io nao carregado na pagina (io undefined)");
        return;
      }
  
      // ✅ evita ficar conectando duas vezes
      if (window.socket && (window.socket.connected || window.socket.connecting)) return;
  
      // ✅ PRODUÇÃO: usa window.SOCKET_URL (definido no index.html)
      // ✅ fallback: window.location.origin (mesmo domínio do WebGL)
      // ✅ DEV: se estiver em localhost/127.0.0.1, cai em http://localhost:3001
      var isLocal =
        (window.location && (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"));
  
      var baseUrl = (window.SOCKET_URL || window.location.origin || "").replace(/\/$/, "");
      if (isLocal && (!window.SOCKET_URL || window.SOCKET_URL.indexOf("localhost") !== -1)) {
        baseUrl = "http://localhost:3001";
      }
  
      // ✅ NÃO use http://localhost em produção
      // ✅ websocket primeiro, fallback polling
      // ✅ path padrão do Socket.IO (bate com seu Nginx location /socket.io/)
      var socket = (window.socket = io(baseUrl, {
        path: "/socket.io",
      ...
      function sendToUnity(method, payload) {
        ...
        // SendMessage só aceita string
        if (typeof payload !== "string") payload = JSON.stringify(payload);
  
        window.unityInstance.SendMessage("NetworkManager", method, payload);
      }
  
      socket.on("connect", function () {
        console.log("✅ Socket conectado:", socket.id, "URL:", baseUrl);
  
        // espera unityInstance existir
        var tries = 0;
        var waitUnity = setInterval(function () {
          tries++;
          if (window.unityInstance && typeof window.unityInstance.SendMessage === "function") {
            clearInterval(waitUnity);
            sendToUnity("SetMySocketId", socket.id);
          }
          if (tries > 200) clearInterval(waitUnity); // ~10s
        }, 50);
      });
  
      socket.on("disconnect", function (reason) {
        console.log("⚠️ Socket desconectado:", reason);
        // opcional: avisar Unity
        // sendToUnity("OnSocketDisconnect", reason || "disconnect");
      });
  
      socket.on("connect_error", function (err) {
        console.error("❌ Socket connect_error:", err && err.message ? err.message : err, "URL:", baseUrl);
        // opcional: avisar Unity
        // sendToUnity("OnSocketError", err && err.message ? err.message : "connect_error");
      });
  
      // ===================== LOBBY =====================
  
      socket.on("lobby_players", function (players) {
        console.log("RECEBI lobby_players:", players);
        // manda array puro (C# faz wrapper {"list": json})
        sendToUnity("OnLobbyPlayers", players);
      });
  
      socket.on("novo_player", function (data) {
        sendToUnity("OnNovoPlayer", data);
      });
  
      socket.on("player_move", function (data) {
        sendToUnity("OnPlayerMove", data);
      });
  
      socket.on("player_saiu", function (socketId) {
        sendToUnity("OnPlayerSaiu", String(socketId || ""));
      });
  
      socket.on("chat", function (data) {
        var payload = data;
        if (typeof data === "string") {
          payload = { socketId: "????", text: data, ts: Date.now() };
        }
        sendToUnity("OnChat", payload);
      });
  
      // ===================== MATCH / SALAS =====================
  
      socket.on("match_found", function (data) {
        sendToUnity("OnMatchFound", data);
      });
  
      socket.on("room_players", function (players) {
        console.log("RECEBI room_players:", players);
        sendToUnity("OnRoomPlayers", players);
      });
  
      socket.on("match_result", function (data) {
        sendToUnity("OnMatchResult", data);
      });
  
      socket.on("wallet", function (data) {
        sendToUnity("OnWallet", data);
      });
  
      socket.on("match_error", function (data) {
        var payload = data;
        if (typeof data === "string") payload = { msg: data };
        sendToUnity("OnMatchError", payload);
      });
      ...
      socket.on("queue_left", function (data) {
        console.log("queue_left:", data);
      });
  
      socket.on("left_match", function (data) {
        console.log("left_match:", data);
      });
    }

  function _SocketJoinQueue(jsonPtr) {
      var socket = window.socket;
      if (!socket) return;
  
      var json = UTF8ToString(jsonPtr);
      try {
        socket.emit("join_queue", JSON.parse(json));
      } catch (e) {
        console.error("SocketJoinQueue JSON invalido:", json, e);
      }
    }

  function _SocketLeaveMatch() {
      var socket = window.socket;
      if (!socket) return;
      socket.emit("leave_match");
    }

  function _SocketLeaveQueue() {
      var socket = window.socket;
      if (!socket) return;
      socket.emit("leave_queue");
    }

  function _SocketMove(jsonPtr) {
      var socket = window.socket;
      if (!socket) return;
  
      var json = UTF8ToString(jsonPtr);
      try {
        socket.emit("move", JSON.parse(json));
      } catch (e) {
        console.error("SocketMove JSON invalido:", json, e);
      }
    }

  function _SocketReportWinner(jsonPtr) {
      var socket = window.socket;
      if (!socket) return;
  
      var json = UTF8ToString(jsonPtr);
      try {
        socket.emit("report_winner", JSON.parse(json));
      } catch (e) {
        console.error("SocketReportWinner JSON invalido:", json, e);
      }
    }

  function _SocketWalletGet() {
      var socket = window.socket;
      if (!socket) return;
      socket.emit("wallet_get");
    }
...
var wasmImports = {
  ...
  "SocketChat": _SocketChat,
  "SocketEntrarLobby": _SocketEntrarLobby,
  "SocketInit": _SocketInit,
  "SocketJoinQueue": _SocketJoinQueue,
  "SocketLeaveMatch": _SocketLeaveMatch,
  "SocketLeaveQueue": _SocketLeaveQueue,
  "SocketMove": _SocketMove,
  "SocketReportWinner": _SocketReportWinner,
  "SocketWalletGet": _SocketWalletGet,
